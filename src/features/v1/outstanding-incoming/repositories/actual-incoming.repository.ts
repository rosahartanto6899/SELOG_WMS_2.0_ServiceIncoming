...
import { ActualIncoming, PlanIncomingHeader, PlanIncomingHeaderAddInfo } from '@/database/entities';
...
export class ActualIncomingRepository {
  ...
  public async findAddInfoByHeaderIds(
    headerIds: string[],
  ): Promise<Array<{ planIncomingHeaderId: string; name: string; value: string }>> {
  ...
  public async softDeleteByHeaderIds(
    ...
    await ActualIncoming.update(
      { isActive: false, deletedBy: userBy, deletedDate: now, modifiedBy: userBy, modifiedDate: now },
      { where: { planIncomingHeaderId: headerIds, isActive: true }, transaction },
    );
  }

  /** A-Delete — record aktif per header (guard: harus ada record actual) */
